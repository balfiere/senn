...
export function PdfViewer({
    ...
    const {
        ...
        error: pdfError,
    } = usePdfLoader(projectId, pdfUrl, projectUpdatedAt);
            ...
            try {
                ...
                switch (eventType) {
                    case 'create': {
                        ...
                        const localAnnotation: LocalPdfAnnotation = {
                            ...
                            embedpdf_annotation_id:
                                annotationData.embedpdf_annotation_id ??
                                annotationId,
                            page_number: annotationData.page_number ?? 1,
                            annotation_type:
                                annotationData.annotation_type ?? 'unknown',
                            ...
                            line_start_ending:
                                annotationData.line_start_ending ?? null,
                            line_end_ending:
                                annotationData.line_end_ending ?? null,
                            contents: annotationData.contents ?? null,
                            comment: annotationData.comment ?? null,
                            in_reply_to_id:
                                annotationData.in_reply_to_id ?? null,
                    ...
                    case 'update': {
                        ...
                        if (!existing) {
                            console.warn(
                                '[PDF Annotation] Cannot update: not found in state',
                                annotationId,
                            );
                            return;
                        }
                        const localAnnotation: LocalPdfAnnotation = {
                            ...
                            annotation_type:
                                annotationData.annotation_type ??
                                existing.annotation_type,
                            position_x:
                                annotationData.position_x ??
                                existing.position_x,
                            position_y:
                                annotationData.position_y ??
                                existing.position_y,
                            width: annotationData.width ?? existing.width,
                            height: annotationData.height ?? existing.height,
                            color:
                                annotationData.color !== undefined
                                    ? annotationData.color
                                    : existing.color,
                            fill_color:
                                annotationData.fill_color !== undefined
                                    ? annotationData.fill_color
                                    : existing.fill_color,
                            stroke_color:
                                annotationData.stroke_color !== undefined
                                    ? annotationData.stroke_color
                                    : existing.stroke_color,
                            opacity: annotationData.opacity ?? existing.opacity,
                            blend_mode:
                                annotationData.blend_mode ??
                                existing.blend_mode,
                            stroke_width:
                                annotationData.stroke_width ??
                                existing.stroke_width,
                            font_size:
                                annotationData.font_size ?? existing.font_size,
                            font_family:
                                annotationData.font_family !== undefined
                                    ? annotationData.font_family
                                    : existing.font_family,
                            line_start_x:
                                annotationData.line_start_x !== undefined
                                    ? annotationData.line_start_x
                                    : existing.line_start_x,
                            line_start_y:
                                annotationData.line_start_y !== undefined
                                    ? annotationData.line_start_y
                                    : existing.line_start_y,
                            line_end_x:
                                annotationData.line_end_x !== undefined
                                    ? annotationData.line_end_x
                                    : existing.line_end_x,
                            line_end_y:
                                annotationData.line_end_y !== undefined
                                    ? annotationData.line_end_y
                                    : existing.line_end_y,
                            line_ending:
                                annotationData.line_ending !== undefined
                                    ? annotationData.line_ending
                                    : existing.line_ending,
                            line_start_ending:
                                annotationData.line_start_ending !== undefined
                                    ? annotationData.line_start_ending
                                    : existing.line_start_ending,
                            line_end_ending:
                                annotationData.line_end_ending !== undefined
                                    ? annotationData.line_end_ending
                                    : existing.line_end_ending,
                            contents:
                                annotationData.contents !== undefined
                                    ? annotationData.contents
                                    : existing.contents,
                            comment:
                                annotationData.comment !== undefined
                                    ? annotationData.comment
                                    : existing.comment,
                            in_reply_to_id:
                                annotationData.in_reply_to_id !== undefined
                                    ? annotationData.in_reply_to_id
                                    : existing.in_reply_to_id,
                            segment_rects:
                                annotationData.segment_rects !== undefined
                                    ? annotationData.segment_rects
                                    : existing.segment_rects,
                            text_align:
                                annotationData.text_align ??
                                existing.text_align,
                            vertical_align:
                                annotationData.vertical_align ??
                                existing.vertical_align,
        ...
        <EmbedPDF
            ...
            onInitialized={async (registry) => {
                ...
                annotationApi.setToolDefaults('highlight', {
                    color:
                        ANNOTATION_COLORS.find((c) => c.name === 'Yellow')
                            ?.value || '#f9e2af',
                    blendMode: PdfBlendMode.Multiply,
                });
                annotationApi.setToolDefaults('underline', {
                    color:
                        ANNOTATION_COLORS.find((c) => c.name === 'Lavender')
                            ?.value || '#b4befe',
                    blendMode: PdfBlendMode.Multiply,
                });
                annotationApi.setToolDefaults('line', {
                    strokeColor:
                        ANNOTATION_COLORS.find((c) => c.name === 'Blue')
                            ?.value || '#89b4fa',
                });
                annotationApi.setToolDefaults('lineArrow', {
                    strokeColor:
                        ANNOTATION_COLORS.find((c) => c.name === 'Maroon')
                            ?.value || '#eba0ac',
                });
                annotationApi.setToolDefaults('square', {
                    strokeColor:
                        ANNOTATION_COLORS.find((c) => c.name === 'Teal')
                            ?.value || '#94e2d5',
                });
                annotationApi.setToolDefaults('freeText', {
                    fontColor:
                        ANNOTATION_COLORS.find((c) => c.name === 'Black')
                            ?.value || '#1e1e2e',
                });
                    ...
                    switch (event.type) {
                        ...
                        case 'delete':
                            ...
                            if (deleteAnnotationId) {
                                ...
                                loadedAnnotationIdsRef.current.delete(
                                    deleteAnnotationId,
                                );
